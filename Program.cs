...
namespace Lab_6_Syed_Hussain_30140466
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            DisplayAllItems();
            SerializeAndSaveFirstItem();
            DisplayFirstItemFromFile();
            SerializeAndSaveCollection();
            DisplayAllItemsFromFile();
        ...
        static void SerializeAndSaveFirstItem()
        {
            var options = new JsonSerializerOptions { WriteIndented = true };
            string jsonString = JsonSerializer.Serialize(elements[0], options);
            File.WriteAllText("first_atom.json", jsonString);
        }

        static void DisplayFirstItemFromFile()
        {
            string jsonString = File.ReadAllText("first_atom.json");
            Atom atom = JsonSerializer.Deserialize<Atom>(jsonString);
            Console.WriteLine(atom.ToString());
        }

        static void SerializeAndSaveCollection()
        {
            var options = new JsonSerializerOptions { WriteIndented = true };
            string jsonString = JsonSerializer.Serialize(elements, options);
            File.WriteAllText("all_atoms.json", jsonString);
        }

        static void DisplayAllItemsFromFile()
        {
            string jsonString = File.ReadAllText("all_atoms.json");
